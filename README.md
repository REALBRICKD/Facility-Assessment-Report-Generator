...
- Users will supply business-context inputs (EMR, patient type, prior coverage/performance, medical coverage) accurately; these are not CMS-derived. For invalid CCN inputs, the application will specify the input format for the user. In the event of other invalid or empty inputs, that cell will be populated by a fallback value such as "N/A"
